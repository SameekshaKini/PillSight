...
def get_transforms(split: str):
    if split == "train":
        return transforms.Compose([
            transforms.Resize((IMAGE_SIZE + 32, IMAGE_SIZE + 32)),  # slightly larger then crop
            ...
            transforms.ColorJitter(
                ...
                saturation=COLOR_JITTER, hue=0.1
            ),
            transforms.ToTensor(),
            transforms.Normalize(MEAN, STD),
            transforms.RandomErasing(p=RANDOM_ERASING, scale=(0.02, 0.2)),
        ])
    else:
        return transforms.Compose([
...
def get_dataloaders(verbose=True):
    ...
    if verbose:
        ...
        print(f"[dataset] Num classes:   {len(class_to_idx)}")
        print(f"[dataset] Class map:     {class_to_idx}")
        print(f"[dataset] Label distribution:\n{df['label_code_id'].value_counts()}")

        # Sanity check: verify a few images actually exist
        missing = 0
        for _, row in df.head(20).iterrows():
            p = os.path.join(IMAGE_ROOT, row["image_path"])
            if not os.path.exists(p):
                print(f"[dataset] MISSING: {p}")
                missing += 1
        if missing == 0:
            print("[dataset] ✓ First 20 image paths verified OK")
        else:
            print(f"[dataset] ✗ {missing}/20 images missing — check IMAGE_ROOT in config.py")

    train_val_df, test_df = train_test_split(
        df, test_size=TEST_SPLIT,
        stratify=df["label_code_id"], random_state=RANDOM_SEED
    )
